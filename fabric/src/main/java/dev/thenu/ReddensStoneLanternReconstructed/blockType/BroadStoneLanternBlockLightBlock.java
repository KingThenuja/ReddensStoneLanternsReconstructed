...
public class BroadStoneLanternBlockLightBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public BroadStoneLanternBlockLightBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                ...
                Block.createCuboidShape(1.0D, 7.0D, 1.0D, 15.0D, 9.0D, 15.0D),
                Block.createCuboidShape(4.0D, 9.0D, 4.0D, 12.0D, 11.0D, 12.0D),
                Block.createCuboidShape(6.0D, 9.0D, 6.0D, 10.0D, 14.0D, 10.0D)
        );
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient()) {
            BroadStoneLanternBlockRightClickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
        }
