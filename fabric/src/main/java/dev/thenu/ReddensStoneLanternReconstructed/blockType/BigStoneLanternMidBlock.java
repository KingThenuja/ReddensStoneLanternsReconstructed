...
public class BigStoneLanternMidBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public BigStoneLanternMidBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                ...
                Block.createCuboidShape(1.0D, 10.0D, 1.0D, 15.0D, 12.0D, 15.0D),
                Block.createCuboidShape(3.0D, 12.0D, 3.0D, 13.0D, 13.0D, 13.0D),
                Block.createCuboidShape(3.0D, 8.0D, 3.0D, 13.0D, 10.0D, 13.0D),
                Block.createCuboidShape(10.0D, 13.0D, 10.0D, 13.0D, 16.0D, 13.0D),
                Block.createCuboidShape(10.0D, 13.0D, 3.0D, 13.0D, 16.0D, 6.0D),
                Block.createCuboidShape(3.0D, 13.0D, 10.0D, 6.0D, 16.0D, 13.0D),
                Block.createCuboidShape(3.0D, 13.0D, 3.0D, 6.0D, 16.0D, 6.0D)
        );
    }
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, WireOrientation sourcePos, boolean notify) {
        ...
        TripleBlockMidTickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
    }

    @Override
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        super.onPlaced(world, pos, state, placer, itemStack);
        BigPlaceProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ(), placer);
    }
