...
public class StonePillarThinABottomBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public StonePillarThinABottomBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                Block.createCuboidShape(2.0D, 3.0D, 2.0D, 14.0D, 4.0D, 14.0D),
                Block.createCuboidShape(6.0D, 13.0D, 6.0D, 10.0D, 16.0D, 10.0D),
                Block.createCuboidShape(1.0D, 2.0D, 1.0D, 15.0D, 3.0D, 15.0D),
                Block.createCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 2.0D, 16.0D),
                Block.createCuboidShape(3.0D, 4.0D, 3.0D, 13.0D, 11.0D, 13.0D),
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, WireOrientation sourcePos, boolean notify) {
        ...
        StonePillarThinATickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
    }
}