...
public class ThinStoneLanternTopLightBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public ThinStoneLanternTopLightBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                ...
                Block.createCuboidShape(2.0D, 7.7D, 2.0D, 14.0D, 9.7D, 14.0D),
                Block.createCuboidShape(4.0D, 9.7D, 4.0D, 12.0D, 11.7D, 12.0D),
                Block.createCuboidShape(6.0D, 10.0D, 6.0D, 10.0D, 13.0D, 10.0D)
        );
    }
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ThinRightClickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
