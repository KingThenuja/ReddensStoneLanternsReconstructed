...
public class BroadStoneLanternTopLightBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public BroadStoneLanternTopLightBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                ...
                Block.createCuboidShape(3.0D, 12.0D, 3.0D, 13.0D, 14.0D, 13.0D),
                Block.createCuboidShape(3.0D, 2.0D, 3.0D, 13.0D, 10.0D, 13.0D),
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            BroadRightClickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
        }
