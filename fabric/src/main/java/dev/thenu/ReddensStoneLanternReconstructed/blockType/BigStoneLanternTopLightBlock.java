...
public class BigStoneLanternTopLightBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public BigStoneLanternTopLightBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                ...
                Block.createCuboidShape(3.0D, 2.0D, 3.0D, 13.0D, 5.0D, 13.0D),
                Block.createCuboidShape(4.0D, 5.0D, 4.0D, 12.0D, 9.0D, 12.0D),
                Block.createCuboidShape(5.0D, 9.0D, 5.0D, 11.0D, 14.0D, 11.0D),
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, WireOrientation sourcePos, boolean notify) {
        ...
        TripleBlockTickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
    }

    @Override
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        BigRightClickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
