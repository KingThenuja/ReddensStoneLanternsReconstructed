...
public class ThinStoneLanternBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public ThinStoneLanternBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                Block.createCuboidShape(3.0D, 0.0D, 3.0D, 13.0D, 2.0D, 13.0D),
                Block.createCuboidShape(4.0D, 2.0D, 4.0D, 12.0D, 3.0D, 12.0D),
                Block.createCuboidShape(6.0D, 3.0D, 6.0D, 10.0D, 14.0D, 10.0D),
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        ThinPlaceProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ(), placer);
    }
