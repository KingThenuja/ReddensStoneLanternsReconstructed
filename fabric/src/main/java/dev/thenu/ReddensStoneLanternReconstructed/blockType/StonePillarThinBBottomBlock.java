...
public class StonePillarThinBBottomBlock extends Block implements Waterloggable {
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public StonePillarThinBBottomBlock(Settings settings) {
        super(settings);
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return VoxelShapes.union(
                Block.createCuboidShape(5.0D, 5.0D, 5.0D, 11.0D, 6.0D, 11.0D),
                Block.createCuboidShape(1.0D, 0.0D, 1.0D, 15.0D, 4.0D, 15.0D),
                Block.createCuboidShape(2.0D, 3.0D, 2.0D, 14.0D, 5.0D, 14.0D),
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, WireOrientation sourcePos, boolean notify) {
        ...
        StonePillarThinBTickProcedure.execute(world, pos.getX(), pos.getY(), pos.getZ());
    }
