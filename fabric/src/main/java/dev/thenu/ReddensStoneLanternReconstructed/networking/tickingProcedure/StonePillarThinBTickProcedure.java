...
public class StonePillarThinBTickProcedure {
    ...
    public static void execute(WorldAccess world, double x, double y, double z) {
        ...
        if (bottomIsPillar) {
            if (!topIsPillar) {
                bs = BlockFile.STONE_PILLAR_THIN_B_TOP.getDefaultState();
            }
        } else if (topIsPillar) {
            bs = BlockFile.STONE_PILLAR_THIN_B_BOTTOM.getDefaultState();
        } else {
            bs = BlockFile.STONE_PILLAR_THIN_B_SHORT.getDefaultState();
        }
