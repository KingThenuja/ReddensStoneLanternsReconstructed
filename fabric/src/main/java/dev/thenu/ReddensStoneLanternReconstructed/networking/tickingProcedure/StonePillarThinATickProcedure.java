...
public class StonePillarThinATickProcedure {
    ...
    public static void execute(WorldAccess world, double x, double y, double z) {
        ...
        if (bottomIsPillar) {
            if (!topIsPillar) {
                bs = BlockFile.STONE_PILLAR_THIN_A_TOP.getDefaultState();
            }
        } else if (topIsPillar) {
            bs = BlockFile.STONE_PILLAR_THIN_A_BOTTOM.getDefaultState();
        } else {
            bs = BlockFile.STONE_PILLAR_THIN_A_SHORT.getDefaultState();
        }
