...
public class StonePillarBigTickProcedure {
    ...
    public static void execute(WorldAccess world, double x, double y, double z) {
        ...
        if (bottomIsPillar) {
            if (topIsPillar) {
                bs = BlockFile.STONE_PILLAR_BIG_MIDDLE.getDefaultState();
            } else {
                bs = BlockFile.STONE_PILLAR_BIG_TOP.getDefaultState();
            }
        } else if (topIsPillar) {
            bs = BlockFile.STONE_PILLAR_BIG_BOTTOM.getDefaultState();
        } else {
            bs = BlockFile.STONE_PILLAR_BIG_SHORT.getDefaultState();
        }
